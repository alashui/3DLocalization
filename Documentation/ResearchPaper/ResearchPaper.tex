...
Before a particle can be weighed, it must have its image features matched with the image features from the actors image. This entails going through the features for both images and seeing which features match best to eachother. In its current implementation, our algorithm only uses SURF features to do the matching between image pairs. We set up the project to also do matching with SIFT and greyscale features, but these methods didn't seem to help our matching attemps so they are currently disabled. Thankfully for us, the SURF feature matching process has already been implemented and is built in standard with the \texttt{OpenCV} libraries. Our group decided to use a \texttt{OpenCV FlannMatcher} object to generate an initial vector of matched features between the two images.
 \[ \verb. Given two vectors of SURF features. \]
 \[ \boldsymbol{F_r} = \verb. [. \mathtt{f_{r1}} \verb., . \mathtt{f_{r2}} \verb-, ..., - \mathtt{f_{rn}} \verb.], .         \mathtt{f_{r}} = \verb. robot image feature . \]
 \[ \boldsymbol{F_p} = \verb. [. \mathtt{f_{p1}} \verb., . \mathtt{f_{p2}} \verb-, ..., - \mathtt{f_{pn}} \verb.], .        \mathtt{f_{p}} = \verb. particle image feature .\]
 \[ Flann(\boldsymbol{F_r} \verb., . \boldsymbol{F_p}) \to \boldsymbol{M} \]
 \[ \verb.Where . \boldsymbol{M} = \mathtt{ [\boldsymbol{m}_0, \boldsymbol{m}_1, ... , \boldsymbol{m}_n]} \verb. and . \boldsymbol{m_i} = \mathtt{ \mathtt{[f_{pi}, f_{ri}]} } \]
...
\[  \boldsymbol{M} = \mathtt{ [\boldsymbol{match}_0, \boldsymbol{match}_1, ... , \boldsymbol{match}_n} ]    \]
