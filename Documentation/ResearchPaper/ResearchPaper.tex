...
First, we have two vectors of SURF features, one for the robot ($\boldsymbol{F_r }$) and one for a specific particle ($\boldsymbol{F_p}$). Each vector contains all features detected in its respective image. There must exist a distance function $f$ that determines the similarity between any two features. 
