...
Before a particle can be weighed, it must have its image features matched with the image features from the actors image. This entails going through the features for both images and seeing which features match best to eachother. In its current implementation, our algorithm only uses SURF features to do the matching between image pairs. We set up the project to also do matching with SIFT and greyscale features, but these methods didn't seem to help our matching attemps so they are currently disabled. Thankfully for us, the SURF feature matching process has already been implemented and is built in standard with the \texttt{OpenCV} libraries. Our group decided to use a \texttt{OpenCV FlannMatcher} object to generate an initial vector of matched features between the two images.
 \\ \\ \verb. Given two vectors of SURF features and a distance function . 
 \[ \boldsymbol{F_r} = \verb. [. \mathtt{f_{r1}} \verb., . \mathtt{f_{r2}} \verb-, ... , - \mathtt{f_{rn}} \verb.] where . \mathtt{f_{r}} = \verb. robot image feature . \]
 \[ \boldsymbol{F_p} = \verb. [. \mathtt{f_{p1}} \verb., . \mathtt{f_{p2}} \verb-, ... , - \mathtt{f_{pn}} \verb.] where .        \mathtt{f_{p}} = \verb. particle image feature .\]
  %\verb. And a distance function that determines how similar two features are .
 \[  f(\mathtt{[f_{pj}, f_{rk}]}) \to \mathtt{distance} \in \mathtt{R^+}   \] \\
  \verb. A match between two features is defined as . 
 \[ \boldsymbol{m} = \mathtt{ \mathtt{[f_{pj}, f_{rk}]} } \verb. such that . \]
 \[ f(\mathtt{[f_{pj}, f_{rk}]}) < f(\mathtt{[f_{pj}, f_{rx}]}) \verb. . \forall \mathtt{ f_{rx}} \in  \boldsymbol{F_r} \verb. . \land \verb. . f(\mathtt{[f_{pj}, f_{rk}]}) < f(\mathtt{[f_{px}, f_{rk}]}) \verb. . \forall  \mathtt{f_{px}} \in \boldsymbol{F_p} \] \\
 % \verb. Conflicts are resolved by taking the match with the lowest total distance . \\ \\
 \verb. The Flann matching function is then defined as .
 \[ Flann(\boldsymbol{F_r} \verb., . \boldsymbol{F_p}) \to \boldsymbol{M} = \mathtt{ [\boldsymbol{m}_0, \boldsymbol{m}_1, ... , \boldsymbol{m}_n ]} \]
 % \[  \verb. Where . \boldsymbol{m_i} = \mathtt{ \mathtt{[f_{pi}, f_{ri}]} }   \]
 % \[ \verb.Where . \boldsymbol{M} = \mathtt{ [\boldsymbol{m_0} \verb., . \boldsymbol{m_1} } \]% \verb., . ... \verb., . \boldsymbol{m_n}]} \verb. and . \boldsymbol{m_i} = \mathtt{ \mathtt{ [f_{pi}, f_{ri}] } } \]

 The \emph{Flann} function will give us a vector of feature matches, but these matches still need to be refined. Using the $k$ nearest neighbors matching algorithm with $k = 2$ and the ratio test\footnote{See Lowe, David G. ``Distinctive Image Features from Scale-Invariant Keypoints." {\it International Journal of Computer Vision} (2004). pg 19-20.  \url{http://www.cs.ubc.ca/~lowe/keypoints/}.} we refine the vector to filter out matches that are below a certain theshold.
\\
